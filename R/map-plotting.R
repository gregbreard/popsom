...
# - nothing
#
map.plot2d <- function(map, x.idx = 1, y.idx = 2, show.dead = F, highlight = NULL) {
  ...
  if (is.null(map$labels)) {
    ...
    # plot data points
    plot(map$data[, x.idx], map$data[, y.idx], col = c("grey"), pch = 20, cex = 0.5)
  } else {
    # get the node labels
    node.labels <- as.numeric(as.factor(map.label.accuracy(map)$neurons))
    pal = c(brewer.pal(max(node.labels, na.rm = T), "Set2"), "white") # get max with NAs
    na.lab <- max(node.labels, na.rm = T) + 1
    node.labels[which(is.na(node.labels))] = na.lab
    
    # plot data points
    plot(map$data[, x.idx], map$data[, y.idx], col = pal[as.factor(map$labels)], pch = 20, cex = 0.5)
  } # end if
  ...
  for (i in 1:dim(map$neurons)[1]) {
    ...
    for (j in 1:length(hood)) {
      ...
      segments(map$neurons[i, x.idx], map$neurons[i, y.idx], map$neurons[l, x.idx], map$neurons[l, y.idx])
    } # end for (j)
  } # end for (i)
  
  # check if we should highlight dead nodes
  if (is.null(highlight) && show.dead)
    highlight <- which(!(1:(map$xdim * map$ydim) %in% unique(map$visual)))
  
  # plot map nodes
  if (is.null(highlight))
    points(map$neurons[, x.idx], map$neurons[, y.idx], col = "black", bg = pal[node.labels], pch = 21)
  else {
    all.n <- 1:dim(map$neurons)[1]
    h <- all.n %in% highlight
    not.h <- !h
    points(map$neurons[not.h, x.idx], map$neurons[not.h, y.idx], col = "black", bg = pal[node.labels[not.h]], pch = 21)
    points(map$neurons[h, x.idx], map$neurons[h, y.idx], col = "darkgoldenrod1", bg = pal[node.labels[h]], pch = 23)
  } # end if
} # end map.plot2d

### map.plot2d - plots the map in 3-dimensions with data
#                point and neurons colored by label
#
# parameters:
# - map is an object returned by map.build
...
# - nothing
#
map.plot3d <- function(map, x.idx = 1, y.idx = 2, z.idx = 3, show.dead = F, highlight = NULL) {
  # need to check if the data has labels
  if (is.null(map$labels)) {
    # get the node labels
    node.labels <- rep(1, dim(map$neurons)[1])
    ...
    # plot data points
    s <- scatterplot3d(map$data[, x.idx], map$data[, y.idx], map$data[, z.idx], color = "grey", pch = 20, cex.symbols = 0.5)
  } else {
    # get the node labels
    node.labels <- as.numeric(as.factor(map.label.accuracy(map)$neurons))
    pal = c(brewer.pal(max(node.labels, na.rm = T), "Set2"), "white") # get max with NAs
    node.labels[which(is.na(node.labels))] = max(node.labels, na.rm = T) + 1
    
    # plot data points
    s <- scatterplot3d(map$data[, x.idx], map$data[, y.idx], map$data[, z.idx], color = pal[as.factor(map$labels)], pch = 20, cex.symbols = 0.5)
  } # end if
  ...
  for (i in 1:dim(map$neurons)[1]) {
    ...
    for (j in 1:length(hood)) {
      ...
      point <- hood[[j]]
      l <- get.map.idx(point[1], point[2], map$xdim)
      p1 <- s$xyz.convert(map$neurons[i, x.idx], map$neurons[i, y.idx], map$neurons[i, z.idx])
      p2 <- s$xyz.convert(map$neurons[l, x.idx], map$neurons[l, y.idx], map$neurons[l, z.idx])
  ...
  # plot map nodes
  if (is.null(highlight)) {
    nodes.2d <- s$xyz.convert(map$neurons[, x.idx], map$neurons[, y.idx], map$neurons[, z.idx])
    points(nodes.2d$x, nodes.2d$y, col = "black", bg = pal[node.labels], pch = 21)
  } else {
    all.n <- 1:dim(map$neurons)[1]
    h <- all.n %in% highlight
    not.h <- !h
    nodes.2d <- s$xyz.convert(map$neurons[not.h, x.idx], map$neurons[not.h, y.idx], map$neurons[not.h, z.idx])
    nodes.h.2d <- s$xyz.convert(map$neurons[h, x.idx], map$neurons[h, y.idx], map$neurons[h, z.idx])
    points(nodes.2d$x, nodes.2d$y, col = "black", bg = pal[node.labels[not.h]], pch = 21)
    points(nodes.h.2d$x, nodes.h.2d$y, col = "darkgoldenrod1", bg = pal[node.labels[h]], pch = 23)
  } # end if
} # end map.plot3d
...
map.label.accuracy <- function(map) {
  ...
  # check how many of the labels of the neurons match the input
  # and how many neurons are mapped to
  acc <- length(which(neuron.l[proj] == labels)) / length(proj)
  bmu <- length(unique(proj)) / length(neuron.l)
  list(neurons = neuron.l, acc = acc, bmu = bmu, fit = (acc * bmu))
